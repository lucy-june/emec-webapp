...
var model= require('../model/mec_vi_offline_content_total.json');
var table = sequelize.define('mec_vi_offline_content_total',model,{freezeTableName:true,createdAt:false,updatedAt:false});
...
//查并计数全部
exports.sum = function(field,qry,tran,lock,cb){
    var options={raw:true};
    if(typeof(tran)!='function'){
        if(tran) options.transaction=tran;
        if(typeof(lock)!='function'){
    ...
    table.sum(field, qry).success(function(res){
        cb(null,res);
    }).error(function (err) {
            cb(err,null);
        })
