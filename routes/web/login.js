...
router.get('/',auth.forward , function(req, res) {
    res.render('index', { "title": titile});
});

//router.get('/yyy',auth.forward , function(req, res) {
...
router.get('/main',auth.backward , function(req, res) {
    ...
    res.render('main', {title: titile, usercode:req.session.usercode, role:req.session.role, istmp:istmp, contractstate:req.session.contractstate,recordstate:req.session.recordstate});
});
